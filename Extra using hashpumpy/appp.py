        ...
        if step == 'generate_mac':
            ...
            if mode == 'insecure':
                ...
                for key_len in range(8, 22):
                    try:
                        new_mac, new_message = hashpumpy.hashpump(mac, original, append, key_len)
                        expected_mac = hashlib.md5(SECRET_KEY + new_message).hexdigest()  # new_message is already bytes
                        full_output.append(f"Trying key length guess: {key_len}")
                        if new_mac == expected_mac:
                            full_output.append("=== SUCCESSFUL FORGERY ===")
                            full_output.append(f"Forged Message: {new_message}")
                            full_output.append(f"Forged MAC: {new_mac}")
                            full_output.append("\n--- Verifying forged message ---\nMAC verified successfully. Message is accepted.")
                            result = {
                                'step': 'forge_mac',
                            ...
                            full_output.append("Forged MAC verification failed.\n")
